...
//棋盘覆盖问题
//有一个2^k*2^k的方格棋盘,恰有一个方格是黑色的,其他为白色,如何用四种L形派覆盖所有的白色格子且任意白色格子只能覆盖一次
...
void slove(char map[100][100],int k, int bx, int by, int sx, int sy){
    //printf("%d %d %d %d %d\n",k,bx,by,sx,sy);
    if(k==1){//we can slove this pro when k = 1
        ...
        for(int i = 0;i<4;i++){
            ...
            printf("%d %d\n",nextx,nexty);
            if(bx>=nextx&&bx<nextx+(int)pow(2,k-1) && by>=nexty&&by<nexty+(int)pow(2,k-1)){//already have a black pixel
                ...
                map[getbx(i,nextx,nexty,k-1)][ getby(i,nextx,nexty,k-1)] = '#';//use '#' to mark virtual pixel
...
int main(){
    ...
    map[bx][by] = '*';//mark for first black pixel
